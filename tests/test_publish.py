...
def test_build_publish_plan_includes_contract_artifacts(tmp_path):
    ...
    plan = build_publish_plan(
        ...
        gcs_prefix="gs://bucket/hk_equity/hk_low_vol_dividend_quality_staging/",
    )

    destinations = [item.destination for item in plan]
    assert len(plan) == 4
    assert destinations == [
        "gs://bucket/hk_equity/hk_low_vol_dividend_quality_staging/hk_low_vol_dividend_quality_factor_snapshot_latest.csv",
        "gs://bucket/hk_equity/hk_low_vol_dividend_quality_staging/hk_low_vol_dividend_quality_factor_snapshot_latest.csv.manifest.json",
        "gs://bucket/hk_equity/hk_low_vol_dividend_quality_staging/hk_low_vol_dividend_quality_ranking_latest.csv",
        "gs://bucket/hk_equity/hk_low_vol_dividend_quality_staging/release_status_summary.json",
    ]


def test_build_publish_plan_includes_validation_report_when_present(tmp_path):
    build_and_write_snapshot(factor_snapshot_path=FACTOR_SNAPSHOT, output_dir=tmp_path)
    validation = validate_snapshot_artifact_pack("hk_low_vol_dividend_quality", tmp_path)
...
def test_publish_artifacts_dry_run_prints_commands(tmp_path, capsys):
    ...
    assert "hk_low_vol_dividend_quality_factor_snapshot_latest.csv" in output

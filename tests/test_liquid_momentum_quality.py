...
from hk_equity_snapshot_pipelines.liquid_momentum_quality import (
...
def test_build_liquid_momentum_feature_snapshot_from_sample_inputs():
    ...
    assert "high_63_gap" in snapshot.columns
    assert snapshot["high_63_gap"].notna().all()
    assert snapshot.loc[snapshot["symbol"] == "00700", "sector"].iloc[0] == "Technology"


def test_build_and_write_liquid_momentum_outputs_artifact_contract(tmp_path):

...
def test_first_snapshot_promotion_plan_scopes_only_retained_candidate():
    ...
    assert plan["profiles_in_scope"] == ["hk_low_vol_dividend_quality_snapshot"]
    assert [profile["profile"] for profile in plan["profiles"]] == ["hk_low_vol_dividend_quality_snapshot"]
...
def test_first_snapshot_promotion_plan_includes_actionable_commands_and_gates():
    plan = build_first_snapshot_promotion_plan(profile="hk_low_vol_dividend_quality_snapshot", platforms=("longbridge",))
    ...
    assert profile["artifact_validation_command"] == (
        ...
        "--profile hk_low_vol_dividend_quality_snapshot --artifact-dir data/output/hk_low_vol_dividend_quality_snapshot --json"
    )
    assert profile["readiness_commands"] == {
        "longbridge": (
            "python scripts/print_snapshot_readiness.py "
            "--profile hk_low_vol_dividend_quality_snapshot --platform longbridge --json"
        )
    }
    assert "--print-template --profile hk_low_vol_dividend_quality_snapshot --platform longbridge --json" in (
...
def test_first_snapshot_promotion_plan_rejects_removed_profile():
    with pytest.raises(ValueError, match="Unsupported first snapshot promotion profile"):
        build_first_snapshot_promotion_plan(profile="hk_free_cash_flow_quality")


def test_first_snapshot_promotion_plan_cli_json():
    completed = subprocess.run(
        [sys.executable, str(SCRIPT), "--profile", "hk_low_vol_dividend_quality_snapshot", "--platform", "ibkr", "--json"],
        check=True,
        capture_output=True,
        text=True,
    )
    ...
    assert payload["profiles_in_scope"] == ["hk_low_vol_dividend_quality_snapshot"]

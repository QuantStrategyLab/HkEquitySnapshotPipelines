...
FACTOR_SNAPSHOT = ROOT / "examples" / "residual_momentum_quality" / "factor_snapshot.sample.csv"


def test_build_and_write_residual_momentum_outputs_artifact_contract(tmp_path):
    result = build_and_write_snapshot(
        factor_snapshot_path=FACTOR_SNAPSHOT,
        output_dir=tmp_path,
    )
    contract = get_profile_contract("hk_residual_momentum")

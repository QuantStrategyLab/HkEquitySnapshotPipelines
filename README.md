...
Feature-snapshot pipeline repository for `HkEquityStrategies`.

## Scope

This repository builds portable snapshot artifacts. It does not submit orders and does not store broker credentials.

The first pipeline builds `hk_blue_chip_leader_rotation` artifacts:
...
Publish the snapshot and manifest to the platform runtime artifact location, then set:

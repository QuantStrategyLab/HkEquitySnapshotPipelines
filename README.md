...
新增 `hk_vcm_cas_microstructure_shock_risk_overlay` 为第三十一个 research-only 候选。它只作为 HKEX VCM trigger、cooling-off price band、POS/CAS auction price limit、closing-auction reference price、order imbalance 和 volatility-interruption context 的市场微观结构冲击 / 执行风险 overlay；不得作为高频 VCM trigger、auction sniping、market-making、latency 或 order-book trading 策略。Promotion 前必须补齐 point-in-time VCM securities list、trigger timestamp、reference price、cooling-off band、POS/CAS reference price、auction status、derivatives VCM coverage、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

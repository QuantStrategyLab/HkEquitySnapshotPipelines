...
### 2026-06-02 update: downside-risk / volatility research gate

新增 `hk_downside_beta_tail_risk_volatility_overlay` 作为 research-only 候选。它只作为 downside beta、semivariance、realized-volatility regime、VaR/CVaR、tail-loss contribution 和 stress-beta 的下行风险降权 / 排除 overlay 进行研究；不得作为单独波动率择时、short-vol、vol-arb、杠杆化 de-risk/re-risk 或做空策略。Promotion 前必须补齐 point-in-time 价格/成交/停牌/公司行动数据、同 universe 与低波红利、quality growth low-vol、lottery、liquidity、financial distress、momentum、value/yield/event/ETF sleeves 的 ablation，且 walk-forward / OOS 证据必须满足最大回撤 <= 30%、每个 OOS fold 最大回撤 <= 30%、至少 3 个独立 OOS fold、单一周期收益贡献 <= 60%、年化收益/最大回撤比 >= 0.50、扣费后仍为正，并保留 dry-run order preview、双语通知、rollout controls 和 operator approval。

Added `hk_downside_beta_tail_risk_volatility_overlay` as the twenty-seventh research-only candidate. It is a downside-risk downweight / exclusion overlay using downside beta, semivariance, realized-volatility regimes, VaR/CVaR, tail-loss contribution, and stress beta; it is not a standalone volatility-timing, short-volatility, volatility-arbitrage, leveraged de-risk/re-risk, or short-selling strategy. Promotion requires point-in-time price/trading/suspension/corporate-action inputs, same-universe ablations against low-vol dividend, quality-growth low-volatility, lottery, liquidity, financial-distress, momentum, value/yield/event/ETF sleeves, and walk-forward/OOS evidence with max drawdown <= 30%, every OOS fold drawdown <= 30%, at least 3 independent OOS folds, max single-period contribution <= 60%, annual-return-to-max-drawdown ratio >= 0.50, positive net-of-cost return, dry-run order previews, bilingual notifications, rollout controls, and operator approval.

### 2026-06-02 update: structured-product warrant / CBBC flow gate

新增 `hk_structured_product_warrant_cbbc_flow_risk_overlay` 作为 research-only 候选。它只作为 HKEX derivative warrants / CBBC issuance、turnover、call/put 或 bull/bear imbalance、maturity/strike clustering、mandatory call event proximity 和 liquidity-provider hedging-stress 的港股结构化产品 flow / tail-risk overlay；不得作为直接交易权证/牛熊证、knockout、market-making 或 issuer-hedging-front-running 策略。Promotion 前必须补齐 point-in-time HKEX 结构化产品上市/发行/到期/正股映射/成交、call price / strike / maturity / MCE distance、liquidity-provider quote availability、正股 VCM/CAS/停牌/成交对齐、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

Added `hk_structured_product_warrant_cbbc_flow_risk_overlay` as the twenty-eighth research-only candidate. It is a HKEX derivative-warrant / CBBC issuance, turnover, call/put or bull/bear imbalance, maturity/strike clustering, mandatory-call-event proximity, and liquidity-provider hedging-stress flow / tail-risk overlay. It is not a direct derivative-warrant, CBBC, knockout, market-making, or issuer-hedging-front-running strategy. Promotion requires point-in-time HKEX structured-product listing/issuance/expiry/underlying/turnover data, call-price / strike / maturity / MCE-distance evidence, liquidity-provider quote-availability evidence, underlying VCM/CAS/suspension/turnover alignment, same-universe ablation, max drawdown <= 30%, dry-run order previews, bilingual notifications, and operator approval.

### 2026-06-02 update: index futures / options sentiment-basis gate

新增 `hk_index_derivatives_futures_options_sentiment_basis_overlay` 作为 research-only 候选。它只作为 HSI / HSTECH / HSCEI futures and options 的 basis、calendar spread、put-call open-interest ratio、implied-volatility skew、term structure、expiry-roll 和 lead-lag 的指数情绪 / regime overlay；不得作为直接期货、期权、spread、volatility、margin 或杠杆交易策略。Promotion 前必须补齐 point-in-time HKEX futures/options price、volume、open interest、contract metadata、expiry calendar、night-session、index/ETF close alignment、same-universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

Added `hk_index_derivatives_futures_options_sentiment_basis_overlay` as the twenty-ninth research-only candidate. It is an HSI / HSTECH / HSCEI futures-and-options basis, calendar-spread, put-call open-interest ratio, implied-volatility skew, term-structure, expiry-roll, and lead-lag index sentiment / regime overlay. It is not a direct futures, options, spread, volatility, margin, or leveraged trading strategy. Promotion requires point-in-time HKEX futures/options price, volume, open-interest, contract-metadata, expiry-calendar, night-session, index/ETF close-alignment, same-universe ablation, max drawdown <= 30%, dry-run order previews, bilingual notifications, and operator approval.

### 2026-06-02 update: VCM / CAS microstructure shock gate

新增 `hk_vcm_cas_microstructure_shock_risk_overlay` 作为 research-only 候选。它只作为 HKEX VCM trigger、cooling-off price band、POS/CAS auction price limit、closing-auction reference price、order imbalance 和 volatility-interruption context 的市场微观结构冲击 / 执行风险 overlay；不得作为高频 VCM trigger、auction sniping、market-making、latency 或 order-book trading 策略。Promotion 前必须补齐 point-in-time VCM securities list、trigger timestamp、reference price、cooling-off band、POS/CAS reference price、auction status、derivatives VCM coverage、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

Added `hk_vcm_cas_microstructure_shock_risk_overlay` as the thirtieth research-only candidate. It is a HKEX VCM trigger, cooling-off price-band, POS/CAS auction price-limit, closing-auction reference-price, order-imbalance, and volatility-interruption microstructure-shock / execution-risk overlay. It is not a high-frequency VCM-trigger, auction-sniping, market-making, latency, or order-book trading strategy. Promotion requires point-in-time VCM securities list, trigger timestamp, reference price, cooling-off band, POS/CAS reference price, auction status, derivatives VCM coverage, same-universe ablation, max drawdown <= 30%, dry-run order previews, bilingual notifications, and operator approval.

### 2026-06-02 update: REIT dividend-spread / rate-sensitivity gate

新增 `hk_reit_dividend_spread_rate_sensitivity_overlay` 作为 research-only 候选。它只作为 HK-listed REIT distribution yield、NAV / GAV discount、gearing、property-sector mix、rental growth、HIBOR / base-rate sensitivity 和 yield-spread regime 的低换手 income / rate-risk overlay；不得作为 leveraged REIT carry、margin、property-developer proxy 或 dividend-capture 策略。Promotion 前必须补齐 point-in-time HKEX REIT universe、SFC REIT Code / product records、distribution / NAV / GAV / gearing / occupancy / rental reversion / debt maturity history、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

### 2026-06-02 update: regulatory enforcement / disciplinary risk gate

新增 `hk_regulatory_enforcement_disciplinary_risk_overlay` 作为 research-only 候选。它只作为 HKEX disciplinary sanctions / enforcement bulletins、SFC enforcement actions、Market Misconduct Tribunal outcomes、restriction notices、false-disclosure cases、director unsuitability statements 和 remedial-action evidence 的治理、披露与市场失当风险 overlay；不得作为 rumour trading、event chasing、litigation arbitrage 或 short-selling 策略。Promotion 前必须补齐 point-in-time HKEX/SFC 官方记录、issuer / director / senior-management mapping、sanction severity、publication timestamp、trading halt / suspension / resumption / liquidity context、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

### 2026-06-02 update: margin financing / collateral forced-selling risk gate

新增 `hk_margin_financing_collateral_forced_selling_risk_overlay` 作为 research-only 候选。它只作为 securities margin financing、broker collateral concentration、marginable-security eligibility、haircut / loan-to-value change、controlling-shareholder share pledge、margin-call pressure 和 forced-selling liquidity gap 的港股去杠杆 / 抵押品风险 overlay；不得作为 leveraged margin trading、forced-liquidation front-running、rumour trading 或 short-selling 策略。Promotion 前必须补齐 point-in-time broker marginable-security / haircut / LTV / concentration evidence、SFC SMF guideline / circular / review evidence、share-pledge / forced-sale disclosure、trading halt / suspension / resumption / VCM-CAS / liquidity context、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

### 2026-06-02 update: liquid large-cap weekly reversal cost-aware gate

新增 `hk_liquid_largecap_weekly_reversal_cost_aware_overlay` 作为 research-only 候选。它只作为 weekly extreme-return reversal / continuance、liquid large-cap universe、fee / spread / slippage、bid-ask bounce、VCM/CAS、suspension 和 momentum interaction 的成本敏感型反转 overlay；不得作为 intraday、高换手、microcap、penny-stock、opening-gap 或纯 contrarian trading 策略。Promotion 前必须补齐 point-in-time weekly return、corporate-action adjusted price、liquidity、spread、fee、slippage、VCM/CAS、suspension/resumption、同 universe momentum / liquidity / volatility / event ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。


### 2026-06-02 update: US ADR / HK secondary-listing lead-lag gate

`hk_us_adr_hk_secondary_listing_lead_lag_overlay` is added as a research-only candidate. It is limited to low-turnover confirmation / de-risking evidence from US ADR overnight or day-session moves, HK secondary-listing close/open alignment, ADR ratio, USD/HKD FX, after-hours / pre-market context, and cross-market price-discovery research. It must not become cross-market arbitrage, conversion arbitrage, pair trading, short selling, margin trading, or after-hours trading.

新增 `hk_us_adr_hk_secondary_listing_lead_lag_overlay` 作为 research-only 候选。它只作为 US ADR overnight / day-session move、HK secondary listing close / open alignment、ADR ratio、USD/HKD FX、after-hours / pre-market context 和 cross-market price-discovery evidence 的低换手 confirmation / de-risking overlay；不得作为 cross-market arbitrage、conversion-arbitrage、pair-trading、short-selling、margin 或 after-hours trading 策略。Promotion 前必须补齐 point-in-time HKEX secondary listing records、issuer company information sheet、ADR ratio / depository data、adjusted HK/US close、FX、corporate actions、holiday mismatch、delisting / conversion constraints、同 universe ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。


### 2026-06-02 update: smart beta factor-regime rotation gate

`hk_smart_beta_factor_regime_rotation_overlay` is added as a research-only candidate. It is limited to low-turnover factor tilt / de-risking evidence across value, dividend/yield, low volatility, quality, momentum, and size sleeves using point-in-time factor index, market-cycle, volatility, investor-sentiment, and macro-regime data. It must not become black-box market timing, high-turnover factor chasing, leveraged trading, shorting, derivatives trading, or intraday trading.

新增 `hk_smart_beta_factor_regime_rotation_overlay` 作为 research-only 候选。它只作为 value、dividend/yield、low-volatility、quality、momentum 和 size sleeves 的低换手 factor tilt / de-risking overlay，使用 point-in-time factor index、market-cycle、volatility、investor-sentiment 和 macro-regime evidence；不得作为黑箱 market timing、高换手 factor chasing、杠杆、做空、衍生品或 intraday trading 策略。Promotion 前必须补齐 point-in-time HSI / HSCI factor methodology、constituents、scores、rebalance buffers、weight caps、factor returns、regime-state labels、同 universe static-factor / QVLM / quality-yield / momentum / ETF ablation、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

### 2026-06-02 update: ESG downside-risk quality gate

`hk_esg_downside_risk_quality_overlay` is added as a research-only candidate. It is limited to low-turnover quality / governance / de-risking evidence from HSI ESG Enhanced methodology, ESG Risk Rating / UNGC / controversial-product screening, constituent exclusions, tilt factors, industry caps, provider-version history, news-sentiment context, institutional-flow context, and downside-risk research. It must not become ESG label-chasing, greenwashing-sensitive marketing, high-turnover controversy trading, shorting, derivatives trading, or standalone ESG timing.

新增 `hk_esg_downside_risk_quality_overlay` 作为 research-only 候选。它只作为 HSI ESG Enhanced methodology、ESG Risk Rating / UNGC / controversial-product screening、constituent exclusion、tilt factor、industry cap、provider-version history、news-sentiment context、institutional-flow context 和 downside-risk research 的低换手 quality / governance / de-risking overlay；不得作为 ESG label-chasing、greenwashing-sensitive marketing、高换手 controversy trading、做空、衍生品或 standalone ESG timing 策略。Promotion 前必须补齐 point-in-time ESG rating / UNGC / controversial-product / provider-version / methodology-change / missing-rating history、HSI ESG Enhanced constituent exclusion / Z-score / tilt-factor / industry-cap / rebalance history、同 universe quality-yield / quality-growth low-vol / regulatory-enforcement / connected-transaction governance / smart-beta / downside-risk / ETF ablation、volatility-spike / greenwashing / controversy stress、回测最大回撤 <= 30%、dry-run order preview、双语通知和 operator approval。

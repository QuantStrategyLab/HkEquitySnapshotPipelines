...
The matrix also exposes `recommended_live_enablement_sequence` for the active work queue, `deferred_proxy_retest_candidates` for quality/yield scaffolds that failed proxy drawdown triage, `research_only_scaffold_sequence` for retained non-active scaffolds, per-row `promotion_scope`, `live_enablement_work_queue`, and `requires_full_backtest_now`, plus `momentum_live_enablement_comparison`, `momentum_live_enablement_policy`, `quality_growth_live_enablement_policy`, `factor_mix_live_enablement_policy`, `policy_value_live_enablement_policy`, `future_research_backlog`, `production_source_audit_policy` with source coverage and stable production-source / quality-report URI requirements, `artifact_provenance_policy`, `evidence_uri_policy`, `evidence_freshness_policy`, `execution_capacity_policy`, `dry_run_order_preview_policy`, `notification_audit_policy`, and `rollout_risk_policy`. Platform switch-plan tooling should only treat `promotion_scope=first_snapshot_live_enablement_candidate` rows as current live-enable work. Research-only and deferred scaffolds keep their strategy-policy notes for future reopening, but they do not require full backtests or live-enable evidence packages now.
...
This validator requires production source audit, artifact-pack validation and publication provenance, walk-forward out-of-sample backtest, platform dry-run order preview, broker permission/fee verification, the required rebalance/event windows, runtime rollout plan, and operator approval. The template and validation result expose the shared `production_source_audit_policy`, `artifact_provenance_policy`, `evidence_uri_policy`, `evidence_freshness_policy`, `execution_capacity_policy`, `dry_run_order_preview_policy`, `rollout_risk_policy`, and `notification_audit_policy`. It rejects missing production source coverage/provenance fields, missing stable production source / quality-report / data-dictionary URIs, missing profile-specific source-audit fields, missing immutable artifact release identity, missing published snapshot/manifest/ranking/release-summary URIs, invalid snapshot sha256, row-count/contract-version mismatch, sample artifact proof gaps, backtests with max drawdown above 30%, annual-return-to-max-drawdown ratio below 0.50, non-positive annual return, less than three out-of-sample years, non-positive excess return versus the required profile benchmark, annualized turnover above the profile cap, missing HK cost-model flags, missing survivorship/look-ahead bias controls, missing per-fold drawdown / parameter-stability / regime-stress / net-cost / stale-price / leverage controls, missing section-level stable `evidence_uri` (`https://`, `gs://`, or `s3://`), missing/stale/future-dated section-level `evidence_generated_at`, missing/over-limit ADV capacity evidence, missing HK board-lot / odd-lot / session-routing / VCM controls, missing dry-run session id, missing stable raw order-preview / quote-snapshot / fee-breakdown artifact URI and sha256 provenance, missing non-sample/redaction/quote-coverage/broker-fee-reconciliation/strategy-decision-reconciliation proof, missing bilingual notification audit fields (`hk_snapshot_live_enablement_dry_run`, EN/ZH-Hans locales, correlation id, redaction, stable delivery-log URI), missing staged-rollout / tripwire / kill-switch / SWT-runbook controls, secret-like query parameters in evidence URIs, missing `risk_approval.approval_reference`, or insufficient paper/dry-run windows.

### 2026-06-02 downside-risk / volatility future candidate

`future_research_backlog` now also includes `hk_downside_beta_tail_risk_volatility_overlay` and `hk_structured_product_warrant_cbbc_flow_risk_overlay`. The artifact contract remains unchanged for existing profiles: if promoted later, this idea needs a new snapshot profile name, new contract version, candidate-specific source-audit fields, and point-in-time downside beta / semivariance / realized-volatility / VaR-CVaR / tail-risk / stress-beta provenance. It must stay non-selectable until same-universe ablation, HK execution/capacity evidence, dry-run order-preview provenance, bilingual notification logs, rollout controls, and operator approval are present.

`future_research_backlog` 现在也包含 `hk_downside_beta_tail_risk_volatility_overlay` 和 `hk_structured_product_warrant_cbbc_flow_risk_overlay`。现有 profile artifact contract 不做原地扩展；若后续提升，必须新建 snapshot profile 名称、contract version、候选专属 source-audit 字段，并补齐 point-in-time downside beta / semivariance / realized-volatility / VaR-CVaR / tail-risk / stress-beta provenance。在同 universe ablation、港股执行/容量证据、dry-run order-preview provenance、双语通知日志、rollout controls 和 operator approval 之前保持 non-selectable。

### 2026-06-02 structured-product warrant / CBBC future candidate

`future_research_backlog` now also includes `hk_structured_product_warrant_cbbc_flow_risk_overlay`. Existing artifact contracts remain unchanged. If promoted later, this idea needs a new snapshot profile name, new contract version, candidate-specific source-audit fields, and point-in-time derivative-warrant / CBBC listing, issuance, expiry, underlying, turnover, call/put or bull/bear, strike, call-price, maturity, mandatory-call-event distance, liquidity-provider quote availability, VCM/CAS, suspension, and same-universe ablation provenance.

`future_research_backlog` 现在也包含 `hk_structured_product_warrant_cbbc_flow_risk_overlay`。现有 artifact contract 不做原地扩展；若后续提升，必须新建 snapshot profile 名称、contract version、候选专属 source-audit 字段，并补齐 point-in-time derivative-warrant / CBBC 上市、发行、到期、正股映射、成交、call/put 或 bull/bear、strike、call price、maturity、mandatory-call-event distance、liquidity-provider quote availability、VCM/CAS、停牌和同 universe ablation provenance。

### 2026-06-02 index futures / options sentiment-basis future candidate

`future_research_backlog` now also includes `hk_index_derivatives_futures_options_sentiment_basis_overlay`. Existing artifact contracts remain unchanged. If promoted later, this idea needs a new snapshot profile name, new contract version, candidate-specific source-audit fields, and point-in-time HSI / HSTECH / HSCEI futures/options price, volume, open interest, basis, put-call ratio, implied-volatility skew, term structure, calendar spread, expiry-roll, night-session, cash/futures close alignment, and same-universe ablation provenance.

`future_research_backlog` 现在也包含 `hk_index_derivatives_futures_options_sentiment_basis_overlay`。现有 artifact contract 不做原地扩展；若后续提升，必须新建 snapshot profile 名称、contract version、候选专属 source-audit 字段，并补齐 point-in-time HSI / HSTECH / HSCEI futures/options price、volume、open interest、basis、put-call ratio、implied-volatility skew、term structure、calendar spread、expiry-roll、night-session、cash/futures close alignment 和同 universe ablation provenance。

### 2026-06-02 VCM / CAS microstructure-shock future candidate
...
### 2026-06-02 regulatory enforcement / disciplinary-risk future candidate

`hk_regulatory_enforcement_disciplinary_risk_overlay` remains research-only and must not mutate existing artifact contracts. If promoted later, it needs a new `regulatory_event_risk_snapshot_overlay` contract with point-in-time HKEX disciplinary-sanction, HKEX enforcement, SFC enforcement, MMT, restriction-notice, false-disclosure, issuer/director mapping, publication timestamp, halt/suspension/resumption, sanction-severity and liquidity-context fields, plus source-audit booleans, bilingual notification metadata, dry-run order-preview provenance and rollout controls.

`hk_regulatory_enforcement_disciplinary_risk_overlay` 目前保持 research-only，不能复用或改写已有 artifact contract。后续如要 promotion，必须新增 `regulatory_event_risk_snapshot_overlay` contract，并包含点时间 HKEX disciplinary sanction、HKEX enforcement、SFC enforcement、MMT、restriction notice、false disclosure、issuer/director mapping、publication timestamp、halt/suspension/resumption、sanction severity 和 liquidity context 字段，以及 source-audit booleans、双语通知 metadata、dry-run order-preview provenance 和 rollout controls。

### 2026-06-02 margin financing / collateral forced-selling future candidate

`hk_margin_financing_collateral_forced_selling_risk_overlay` remains research-only and must not mutate existing artifact contracts. If promoted later, it needs a new `margin_collateral_risk_snapshot_overlay` contract with point-in-time marginable-security, haircut, LTV, broker collateral-concentration, SFC SMF guideline, share-pledge, margin-call proxy, forced-sale disclosure, trading-halt/suspension/resumption, VCM/CAS and liquidity-context fields, plus source-audit booleans, bilingual notification metadata, dry-run order-preview provenance and rollout controls.

`hk_margin_financing_collateral_forced_selling_risk_overlay` 目前保持 research-only，不能复用或改写已有 artifact contract。后续如要 promotion，必须新增 `margin_collateral_risk_snapshot_overlay` contract，并包含点时间 marginable security、haircut、LTV、broker collateral concentration、SFC SMF guideline、share pledge、margin-call proxy、forced-sale disclosure、trading halt / suspension / resumption、VCM/CAS 和 liquidity context 字段，以及 source-audit booleans、双语通知 metadata、dry-run order-preview provenance 和 rollout controls。

### 2026-06-02 liquid large-cap weekly reversal future candidate

`hk_liquid_largecap_weekly_reversal_cost_aware_overlay` remains research-only and must not mutate existing artifact contracts. If promoted later, it needs a new `cost_aware_reversal_snapshot_overlay` contract with point-in-time weekly extreme-return, reversal / continuance, liquid-large-cap universe, fee, spread, slippage, bid-ask-bounce, VCM/CAS, suspension/resumption, momentum-interaction and same-universe-ablation fields, plus source-audit booleans, bilingual notification metadata, dry-run order-preview provenance and rollout controls.

`hk_liquid_largecap_weekly_reversal_cost_aware_overlay` 目前保持 research-only，不能复用或改写已有 artifact contract。后续如要 promotion，必须新增 `cost_aware_reversal_snapshot_overlay` contract，并包含点时间 weekly extreme-return、reversal / continuance、liquid-large-cap universe、fee、spread、slippage、bid-ask-bounce、VCM/CAS、suspension/resumption、momentum-interaction 和 same-universe-ablation 字段，以及 source-audit booleans、双语通知 metadata、dry-run order-preview provenance 和 rollout controls。


### 2026-06-02 US ADR / HK secondary-listing lead-lag future candidate

`hk_us_adr_hk_secondary_listing_lead_lag_overlay` remains research-only and must not mutate existing artifact contracts. If promoted later, it needs a new `cross_market_lead_lag_snapshot_overlay` contract with point-in-time ADR/HK mapping, ADR ratio, primary/secondary listing status, issuer information sheet, US close, after-hours/pre-market context, HK open/close, USD/HKD FX, corporate actions, conversion/fungibility constraints, holiday/delisting risk, VCM/CAS and broker-permission fields, plus source-audit booleans, bilingual notification metadata, dry-run order-preview provenance and rollout controls.

`hk_us_adr_hk_secondary_listing_lead_lag_overlay` 目前保持 research-only，不能复用或改写已有 artifact contract。后续如要 promotion，必须新增 `cross_market_lead_lag_snapshot_overlay` contract，并包含 point-in-time ADR/HK mapping、ADR ratio、primary/secondary listing status、issuer information sheet、US close、after-hours/pre-market context、HK open/close、USD/HKD FX、corporate actions、conversion/fungibility constraints、holiday/delisting risk、VCM/CAS 和 broker-permission 字段，以及 source-audit booleans、双语通知 metadata、dry-run order-preview provenance 和 rollout controls。


### 2026-06-02 smart beta factor-regime rotation future candidate

`hk_smart_beta_factor_regime_rotation_overlay` remains research-only and must not mutate existing artifact contracts. If promoted later, it needs a new `factor_regime_rotation_snapshot_overlay` contract with point-in-time factor-index methodology version, constituent, factor-score, factor-return, rebalance-buffer, weight-cap, market-cycle, volatility, investor-sentiment, macro-regime, factor-correlation, turnover, sector-concentration, cost, capacity and regime-classifier-stability fields, plus source-audit booleans, bilingual notification metadata, dry-run order-preview provenance and rollout controls.

`hk_smart_beta_factor_regime_rotation_overlay` 目前保持 research-only，不能复用或改写已有 artifact contract。后续如要 promotion，必须新增 `factor_regime_rotation_snapshot_overlay` contract，并包含 point-in-time factor-index methodology version、constituent、factor-score、factor-return、rebalance-buffer、weight-cap、market-cycle、volatility、investor-sentiment、macro-regime、factor-correlation、turnover、sector-concentration、cost、capacity 和 regime-classifier-stability 字段，以及 source-audit booleans、双语通知 metadata、dry-run order-preview provenance 和 rollout controls。

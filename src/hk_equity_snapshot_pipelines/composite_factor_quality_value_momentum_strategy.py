...
STATUS_ICON = "🇭🇰"
PROFILE_NAME = "hk_composite_factor_quality_value_momentum"
SAFE_HAVEN = "02800"
...
DEFAULT_MIN_MOM_12M_TO_HIGH = -0.35
DEFAULT_MIN_SMA200_GAP = -0.05
...
SNAPSHOT_CONTRACT_VERSION = "hk_composite_factor_quality_value_momentum.factor_snapshot.v1"
...
def _candidate_frame(
    ...
    *,
    ...
    return frame.loc[
        ...
        & frame["mom_12m_to_high"].ge(float(min_mom_12m_to_high))
...
def score_candidates(
    ...
    *,
    ...
    min_mom_12m_to_high: float = DEFAULT_MIN_MOM_12M_TO_HIGH,
    ...
    eligible = _candidate_frame(
        ...
        min_mom_12m_to_high=float(min_mom_12m_to_high),
    ...
    eligible["quality_score"] = (
        _zscore(eligible["roe_ttm"]) * 0.45
        - _zscore(eligible["earnings_variability_3y"]) * 0.30
        - _zscore(eligible["debt_to_equity"]) * 0.25
    )
    eligible["value_score"] = (
        _zscore(eligible["book_to_price"]) * 0.35
        + _zscore(eligible["earnings_yield"]) * 0.35
        + _zscore(eligible["free_cash_flow_yield"]) * 0.30
    )
    eligible["momentum_score"] = _zscore(eligible["mom_12m_to_high"]) * 0.70 + _zscore(eligible["sma200_gap"]) * 0.30
    eligible["low_vol_score"] = (
        -_zscore(eligible["realized_vol_252"]) * 0.45
        - _zscore(eligible["beta_252"]) * 0.30
        - _zscore(eligible["maxdd_abs"]) * 0.25
    )
    eligible["score"] = (
        eligible["quality_score"] * 0.30
        + eligible["value_score"] * 0.25
        + eligible["momentum_score"] * 0.25
        + eligible["low_vol_score"] * 0.20
    )
    ...
    ranked = eligible.sort_values(
        by=["score", "quality_score", "momentum_score", "adv20_hkd", "symbol"],
...
def compute_signals(
    ...
    **kwargs: Any,
    ...
    signal_desc = (
        f"hk composite factor qvm regime={metadata['regime']} breadth={metadata['breadth_ratio']:.1%} "

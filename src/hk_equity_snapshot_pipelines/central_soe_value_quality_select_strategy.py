...
STATUS_ICON = "🇭🇰"
PROFILE_NAME = "hk_central_soe_value_quality_select"
SAFE_HAVEN = "02800"
DEFAULT_HOLDINGS_COUNT = 10
DEFAULT_SINGLE_NAME_CAP = 0.08
DEFAULT_SECTOR_CAP = 0.24
DEFAULT_MIN_ADV20_HKD = 50_000_000.0
DEFAULT_MIN_MARKET_CAP_HKD = 10_000_000_000.0
DEFAULT_MIN_GOVERNMENT_OWNERSHIP_PCT = 0.10
DEFAULT_MAX_DEBT_TO_EQUITY = 5.0
...
SNAPSHOT_CONTRACT_VERSION = "hk_central_soe_value_quality_select.factor_snapshot.v1"
...
OPTIONAL_FACTOR_COLUMNS = frozenset(
    {
        ...
        "southbound_eligible",
        "lot_size",
        "government_shareholder_type",
        "policy_event_risk_flag",
...
def _candidate_frame(
    ...
    *,
    ...
    min_government_ownership_pct: float,
    ...
    return frame.loc[
        ...
        & frame["central_soe_flag"]
        & frame["government_largest_shareholder"]
        & ~frame["policy_event_risk_flag"]
        & ~frame["corporate_action_flag"]
        & frame["adv20_hkd"].ge(float(min_adv20_hkd))
        & frame["market_cap_hkd"].ge(float(min_market_cap_hkd))
        & frame["government_ownership_pct"].ge(float(min_government_ownership_pct))
        & frame["dividend_yield_net"].ge(0.0)
        & frame["earnings_yield"].gt(0.0)
        & frame["book_to_price"].gt(0.0)
...
def score_candidates(
    ...
    *,
    ...
    min_government_ownership_pct: float = DEFAULT_MIN_GOVERNMENT_OWNERSHIP_PCT,
    ...
    eligible = _candidate_frame(
        ...
        min_government_ownership_pct=float(min_government_ownership_pct),
    ...
    eligible["value_factor_score"] = (
        _zscore(eligible["value_score"]) * 0.40
        + _zscore(eligible["earnings_yield"]) * 0.25
        + _zscore(eligible["book_to_price"]) * 0.25
        + _zscore(eligible["dividend_yield_net"]) * 0.10
    )
    eligible["quality_factor_score"] = (
        _zscore(eligible["quality_score"]) * 0.45
        + _zscore(eligible["roe_ttm"]) * 0.35
        - _zscore(eligible["debt_to_equity"]) * 0.20
    )
    eligible["risk_factor_score"] = (
        _zscore(eligible["low_volatility_score"]) * 0.50
        - _zscore(eligible["realized_vol_252"]) * 0.30
        - _zscore(eligible["maxdd_abs"]) * 0.20
    )
    eligible["trend_factor_score"] = _zscore(eligible["momentum_score"]) * 0.70 + _zscore(eligible["sma200_gap"]) * 0.30
    eligible["ownership_alignment_score"] = _zscore(eligible["government_ownership_pct"])
    eligible["score"] = (
        eligible["value_factor_score"] * 0.35
        + eligible["quality_factor_score"] * 0.30
        + eligible["risk_factor_score"] * 0.20
        + eligible["trend_factor_score"] * 0.10
        + eligible["ownership_alignment_score"] * 0.05
    )
    ...
    ranked = eligible.sort_values(
        by=["score", "value_factor_score", "quality_factor_score", "adv20_hkd", "symbol"],
...
def compute_signals(
    ...
    **kwargs: Any,
    ...
    signal_desc = (
        f"hk central soe value quality select regime={metadata['regime']} "

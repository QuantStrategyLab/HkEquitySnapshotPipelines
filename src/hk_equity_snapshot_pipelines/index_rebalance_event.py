...
from .contracts import HK_INDEX_REBALANCE_EVENT_PROFILE, get_profile_contract
from .index_rebalance_event_strategy import compute_signals, score_candidates
...
def build_and_write_snapshot(
    *,
    ...
    min_add_probability: float = 0.0,
    max_estimated_slippage_bps: float = 10_000.0,
) -> SnapshotBuildResult:
    contract = get_profile_contract(HK_INDEX_REBALANCE_EVENT_PROFILE)
    artifact_paths = contract.artifact_paths(output_dir)
    snapshot = _read_csv(event_snapshot_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--event-snapshot", required=True, help="CSV with hk_index_rebalance_event columns")
    ...
    parser.add_argument("--min-add-probability", type=float, default=0.0)
    parser.add_argument("--max-estimated-slippage-bps", type=float, default=10_000.0)
    args = parser.parse_args(argv)

    result = build_and_write_snapshot(
        event_snapshot_path=args.event_snapshot,
        output_dir=args.output_dir,
        min_adv20_hkd=args.min_adv20_hkd,
        min_market_cap_hkd=args.min_market_cap_hkd,
        min_add_probability=args.min_add_probability,
        max_estimated_slippage_bps=args.max_estimated_slippage_bps,
    )

...
from .first_snapshot_evidence_profiles import normalize_first_snapshot_profile
...
FIRST_SNAPSHOT_BACKTEST_DRAFT_VERSION = "hk_first_snapshot_walk_forward_backtest_draft.v1"
DEFAULT_OUTPUT_DIR = Path("data/output/first_snapshot_walk_forward_backtests")
...
def analyze_first_snapshot_backtest_summary(profile: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    max_turnover = get_max_allowed_annualized_turnover(selected_profile)
    ...
    required_benchmark = get_required_benchmark_symbol(selected_profile)
    ...
    return {
        "draft_version": FIRST_SNAPSHOT_BACKTEST_DRAFT_VERSION,
        ...
        "required_benchmark_symbol": required_benchmark,
        "max_allowed_annualized_turnover": max_turnover,
        "errors": errors,
        "warnings": warnings,
    }


def build_first_snapshot_backtest_evidence_draft(
    *,
    profile: str,
    ...
    analysis = analyze_first_snapshot_backtest_summary(selected_profile, summary)
    ...
    draft["operator_note"] = (
        ...
        "fold metrics, parameter sensitivity, HK cost model, and PIT/no-lookahead evidence are attached."
    )
    return {
        "draft_version": FIRST_SNAPSHOT_BACKTEST_DRAFT_VERSION,
...
def write_first_snapshot_backtest_evidence_draft(
    *,
    profile: str,
    summary: dict[str, Any],
    ...
    draft_path = resolved_output_dir / "walk_forward_backtest.draft.json"
    summary_path = resolved_output_dir / "walk_forward_backtest_summary_validation.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Draft walk-forward backtest evidence for a first HK snapshot profile.")
    ...
    if args.json:
        payload = build_first_snapshot_backtest_evidence_draft(**kwargs)
    else:

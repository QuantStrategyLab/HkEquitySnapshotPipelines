...
from .contracts import HK_SOUTHBOUND_FLOW_MOMENTUM_PROFILE, get_profile_contract
from .southbound_flow_momentum_strategy import compute_signals, score_candidates
...
def build_and_write_snapshot(
    *,
    flow_snapshot_path: str | Path,
    output_dir: str | Path,
    min_adv20_hkd: float = 0.0,
    min_market_cap_hkd: float = 0.0,
    min_flow_zscore: float = 0.0,
    min_flow_persistence: float = 0.0,
) -> SnapshotBuildResult:
    contract = get_profile_contract(HK_SOUTHBOUND_FLOW_MOMENTUM_PROFILE)
    artifact_paths = contract.artifact_paths(output_dir)
    snapshot = _read_csv(flow_snapshot_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--flow-snapshot", required=True, help="CSV with hk_southbound_flow_momentum flow columns")
    ...
    parser.add_argument("--min-flow-zscore", type=float, default=0.0)
    parser.add_argument("--min-flow-persistence", type=float, default=0.0)
    args = parser.parse_args(argv)

    result = build_and_write_snapshot(
        flow_snapshot_path=args.flow_snapshot,
        output_dir=args.output_dir,
        min_adv20_hkd=args.min_adv20_hkd,
        min_market_cap_hkd=args.min_market_cap_hkd,
        min_flow_zscore=args.min_flow_zscore,
        min_flow_persistence=args.min_flow_persistence,
    )

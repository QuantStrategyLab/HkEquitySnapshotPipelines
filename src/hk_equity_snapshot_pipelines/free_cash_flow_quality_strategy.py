...
STATUS_ICON = "🇭🇰"
PROFILE_NAME = "hk_free_cash_flow_quality"
SAFE_HAVEN = "02800"
...
DEFAULT_MIN_FREE_CASH_FLOW_YIELD = 0.02
DEFAULT_MIN_REVENUE_GROWTH_TTM = -0.20
...
SNAPSHOT_CONTRACT_VERSION = "hk_free_cash_flow_quality.factor_snapshot.v1"
...
def _candidate_frame(
    ...
    *,
    ...
    min_free_cash_flow_yield: float,
    min_revenue_growth_ttm: float,
    ...
    return frame.loc[
        ...
        & frame["free_cash_flow_hkd"].gt(0.0)
        & frame["enterprise_value_hkd"].gt(0.0)
        & frame["free_cash_flow_yield"].ge(float(min_free_cash_flow_yield))
        & frame["roe_ttm"].gt(0.0)
        & frame["revenue_growth_ttm"].ge(float(min_revenue_growth_ttm))
...
def score_candidates(
    ...
    *,
    ...
    min_free_cash_flow_yield: float = DEFAULT_MIN_FREE_CASH_FLOW_YIELD,
    min_revenue_growth_ttm: float = DEFAULT_MIN_REVENUE_GROWTH_TTM,
    ...
    eligible = _candidate_frame(
        ...
        min_free_cash_flow_yield=float(min_free_cash_flow_yield),
        min_revenue_growth_ttm=float(min_revenue_growth_ttm),
    ...
    eligible["fcf_score"] = _zscore(eligible["free_cash_flow_yield"]) * 0.70 + _optional_zscore(
        eligible,
        "fcf_yield_percentile_3y",
    ) * 0.30
    eligible["quality_score"] = _zscore(eligible["roe_ttm"]) * 0.55 + _zscore(eligible["revenue_growth_ttm"]) * 0.20
    eligible["quality_score"] += _optional_zscore(eligible, "earnings_yield") * 0.15
    eligible["quality_score"] -= _optional_zscore(eligible, "debt_to_equity") * 0.10
    ...
    ranked = eligible.sort_values(
        by=["score", "free_cash_flow_yield", "roe_ttm", "adv20_hkd", "symbol"],
...
def compute_signals(factor_snapshot: Any, current_holdings: Any, *, safe_haven: str = SAFE_HAVEN, **kwargs: Any):
    ...
    signal_desc = (
        f"hk free cash flow quality regime={metadata['regime']} breadth={metadata['breadth_ratio']:.1%} "

...
SIGNAL_SOURCE = "valuation_snapshot"
STATUS_ICON = "🇭🇰"
PROFILE_NAME = "hk_ah_premium_relative_value"
SAFE_HAVEN = "02800"
DEFAULT_HOLDINGS_COUNT = 8
DEFAULT_SINGLE_NAME_CAP = 0.12
DEFAULT_SECTOR_CAP = 0.30
DEFAULT_MIN_H_ADV20_HKD = 50_000_000.0
DEFAULT_MIN_H_MARKET_CAP_HKD = 10_000_000_000.0
DEFAULT_MIN_AH_PREMIUM_PCT = 0.10
DEFAULT_MIN_AH_PREMIUM_PERCENTILE_3Y = 0.50
DEFAULT_MAX_H_VOL_63 = 0.08
...
SNAPSHOT_CONTRACT_VERSION = "hk_ah_premium_relative_value.valuation_snapshot.v1"
REQUIRE_SNAPSHOT_MANIFEST = True

REQUIRED_VALUATION_COLUMNS = frozenset(
    {
        "symbol",
        "a_symbol",
        "sector",
...
OPTIONAL_VALUATION_COLUMNS = frozenset(
    {
        "as_of",
        "snapshot_date",
        "eligible",
...
def _normalize_a_symbol(value: Any) -> str:
    if pd.isna(value):
        return ""
    text = str(value or "").strip().upper()
...
def _to_frame(valuation_snapshot: Any) -> pd.DataFrame:
    frame = valuation_snapshot.copy() if isinstance(valuation_snapshot, pd.DataFrame) else pd.DataFrame(list(valuation_snapshot))
    if frame.empty:
        raise ValueError("valuation_snapshot must contain at least one row")
    missing = REQUIRED_VALUATION_COLUMNS - set(frame.columns)
    if missing:
        missing_text = ", ".join(sorted(missing))
        raise ValueError(f"valuation_snapshot missing required columns: {missing_text}")

    frame["symbol"] = frame["symbol"].map(normalize_symbol)
    frame["a_symbol"] = frame["a_symbol"].map(_normalize_a_symbol)
    ...
    frame["connect_eligible_h"] = frame["connect_eligible_h"].map(_coerce_bool)
    frame["connect_eligible_a"] = frame["connect_eligible_a"].map(_coerce_bool)
    ...
    if "southbound_holding_pct_change_20d" not in frame.columns:
        frame["southbound_holding_pct_change_20d"] = 0.0
    if "ah_premium_change_20d" not in frame.columns:
        frame["ah_premium_change_20d"] = 0.0
    if "h_liquidity_score" not in frame.columns:
        frame["h_liquidity_score"] = frame["h_adv20_hkd"]

    numeric_columns = (REQUIRED_VALUATION_COLUMNS | OPTIONAL_VALUATION_COLUMNS) - {
        "symbol",
        "a_symbol",
        "sector",
        "as_of",
...
def _candidate_frame(
    ...
    *,
    safe_haven: str,
    min_h_adv20_hkd: float,
    min_h_market_cap_hkd: float,
    min_ah_premium_pct: float,
    min_ah_premium_percentile_3y: float,
    max_h_vol_63: float,
    ...
    return frame.loc[
        ...
        & frame["eligible"]
        & frame["connect_eligible_h"]
        & frame["connect_eligible_a"]
        & ~frame["corporate_action_flag"]
        & frame["h_adv20_hkd"].ge(float(min_h_adv20_hkd))
        & frame["h_market_cap_hkd"].ge(float(min_h_market_cap_hkd))
        & frame["ah_premium_pct"].ge(float(min_ah_premium_pct))
        & frame["ah_premium_percentile_3y"].ge(float(min_ah_premium_percentile_3y))
        & frame["h_mom_6m"].gt(0.0)
        & frame["h_sma200_gap"].gt(0.0)
        & frame["h_vol_63"].le(float(max_h_vol_63))
...
def score_candidates(
    ...
    *,
    ...
    min_h_adv20_hkd: float = DEFAULT_MIN_H_ADV20_HKD,
    min_h_market_cap_hkd: float = DEFAULT_MIN_H_MARKET_CAP_HKD,
    min_ah_premium_pct: float = DEFAULT_MIN_AH_PREMIUM_PCT,
    min_ah_premium_percentile_3y: float = DEFAULT_MIN_AH_PREMIUM_PERCENTILE_3Y,
    max_h_vol_63: float = DEFAULT_MAX_H_VOL_63,
    max_suspension_days_63: int = DEFAULT_MAX_SUSPENSION_DAYS_63,
    hold_bonus: float = DEFAULT_HOLD_BONUS,
) -> pd.DataFrame:
    frame = _to_frame(valuation_snapshot)
    eligible = _candidate_frame(
        frame,
        safe_haven=safe_haven,
        min_h_adv20_hkd=float(min_h_adv20_hkd),
        min_h_market_cap_hkd=float(min_h_market_cap_hkd),
        min_ah_premium_pct=float(min_ah_premium_pct),
        min_ah_premium_percentile_3y=float(min_ah_premium_percentile_3y),
        max_h_vol_63=float(max_h_vol_63),
        max_suspension_days_63=int(max_suspension_days_63),
    )
    if eligible.empty:
        return pd.DataFrame(columns=["rank", "symbol", "sector", "score", "eligible"])

    eligible["premium_narrowing_score"] = -eligible["ah_premium_change_20d"]
    eligible["score"] = (
        _zscore(eligible["ah_premium_pct"]) * 0.25
        + _zscore(eligible["ah_premium_percentile_3y"]) * 0.15
        + _zscore(eligible["premium_narrowing_score"]) * 0.15
        + _zscore(eligible["h_mom_6m"]) * 0.15
        + _zscore(eligible["h_sma200_gap"]) * 0.10
        + _zscore(eligible["southbound_holding_pct_change_20d"]) * 0.10
        + _zscore(eligible["h_liquidity_score"]) * 0.05
        - _zscore(eligible["h_vol_63"]) * 0.05
    )
    ...
    ranked = eligible.sort_values(
        by=["score", "ah_premium_pct", "h_adv20_hkd", "symbol"],
...
def _resolve_stock_exposure(
    ...
    *,
    ...
    breadth_ratio = float((ranked["h_sma200_gap"] > 0).mean()) if not ranked.empty else 0.0
...
def extract_managed_symbols(valuation_snapshot: Any, *, safe_haven: str = SAFE_HAVEN) -> tuple[str, ...]:
    frame = _to_frame(valuation_snapshot)
    safe_haven = normalize_symbol(safe_haven)
...
def compute_signals(valuation_snapshot: Any, current_holdings: Any, *, safe_haven: str = SAFE_HAVEN, **kwargs: Any):
    kwargs.pop("translator", None)
    kwargs.pop("signal_text_fn", None)
    kwargs.pop("execution_cash_reserve_ratio", None)
    managed_symbols = extract_managed_symbols(valuation_snapshot, safe_haven=safe_haven)
    ...
    signal_desc = (
        f"hk ah premium relative value regime={metadata['regime']} breadth={metadata['breadth_ratio']:.1%} "
